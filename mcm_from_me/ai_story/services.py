...
def generate_ai_card_image(card: JourneyCard) -> JourneyCard:
    ...
    try:
        ...
        prompt = (
            f"첫 번째 이미지 속 인물의 성별, 대략적인 나이대, 헤어스타일 같은 전체적인 분위기만 참고하고, "
            f"실제 얼굴을 사실적으로 그대로 재현하지는 마. 이목구비와 피부는 고급 매거진 화보 모델처럼 "
            f"매끄럽고 균형 잡히게 이상적으로 재해석해서, 사실적인 인물 사진이 아니라 세련된 AI 아트 화보 "
            f"일러스트처럼 보이게 그려줘. 원본 사진을 그대로 오려 붙인 합성처럼 보이면 안 돼. "
            f"고급 매거진 화보를 위해 전문 스튜디오에서 촬영한 것처럼 조명, 톤, 화질을 전체적으로 새롭게 "
            f"스타일링해줘. 상반신 위주의 인물 포트레이트 구도로, 인물은 카메라를 정면 또는 살짝 비스듬히 "
            f"바라보고, 배경은 {moment_desc}이 은은하게 아웃포커스(보케) 처리되도록 해줘. "
            f"{product_name} 가방을 {carry_opt} 방식으로 들고 있는 자연스러운 모습으로 표현해줘. "
            f"{detail_opt} 디테일이 잘 보이게 해줘. "
            f"시네마틱한 색감과 부드러운 조명의 화보 같은 분위기로, 전체적으로 고급스럽고 세련된 톤을 유지해줘."
        )
